...
def render(
    ...
    *,
    ...
    """Render an outer view or remove the camera-side half for internal QA."""
    ...
    size = high - low
    if cutaway:
        # Stop before the first one-third-width rail so the cell profile remains
        # visible; a centre cut would look like a solid rail rather than a cell QA.
        cut_x = low[0] + 0.25 * size[0]
        keep = triangles.mean(axis=1)[:, 0] <= cut_x
        triangles = triangles[keep]
        normals = normals[keep]
    camera_offset = (
        np.array((size[0] * 2.4, 0.0, 0.0))
        if cutaway
        else np.array((size[0] * 2.4, -size[1] * 0.70, size[2] * 2.1))
    )
    camera = centre + camera_offset
...
def main() -> int:
    ...
    render(arguments.source, arguments.output, cutaway=arguments.cutaway)
    return 0
