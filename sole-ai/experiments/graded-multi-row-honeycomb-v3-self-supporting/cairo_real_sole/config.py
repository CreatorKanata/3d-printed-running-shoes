# cairo_real_sole.config: V3.1 honeycomb, material, and mesh settings.
"""Centralise tunables for the self-supporting honeycomb sole."""
...
class SoleCellConfig:
    """Millimetre settings for the full-size TPE-83A V3.1 candidate."""

    wall_mm: float = 1.5
    dense_wall_mm: float = 1.8
    honeycomb_half_width_mm: float = 4.0
    honeycomb_half_height_mm: float = 6.5
    honeycomb_vertical_half_edge_mm: float = 2.0
    honeycomb_bin_mm: float = 12.0
    longitudinal_rail_count: int = 2
    merge_vertical_source_intervals: bool = False
    plate_slot_enabled: bool = False
    plate_slot_expected_mm: float = 2.2
    plate_slot_wall_mm: float = 1.5
    ...
    rail_height_fraction_low: float = 0.0
    rail_height_fraction_high: float = 1.0
    ...
    def validate(self, step_mm: float) -> None:
        ...
        diagonal_rise = self.honeycomb_half_height_mm - self.honeycomb_vertical_half_edge_mm
        diagonal_angle = math.degrees(math.atan2(diagonal_rise, self.honeycomb_half_width_mm))
        if min(self.honeycomb_half_width_mm, diagonal_rise) < 2.0 * self.dense_wall_mm:
            raise ValueError("Honeycomb openings need space between 1.8 mm walls.")
        if diagonal_angle < 45.0:
            raise ValueError("Every non-vertical honeycomb wall must rise at least 45 degrees.")
        if not self.wall_mm <= self.dense_wall_mm <= 2.0:
            raise ValueError("V3.1 walls must grade only from 1.5 to at most 2.0 mm.")
        if self.longitudinal_rail_count not in (0,) and self.longitudinal_rail_count < 2:
            raise ValueError("Rail count must be zero or at least two.")
        if self.plate_slot_enabled and self.merge_vertical_source_intervals:
            raise ValueError("A preserved plate slot cannot be vertically merged.")
        if self.plate_slot_wall_mm < self.wall_mm:
            raise ValueError("The plate-slot perimeter cannot be thinner than 1.5 mm.")
        ...
        if not (
            ...
            raise ValueError("Rails must stay within the honeycomb height.")
        fractions = (
        ...
        if self.material_density_g_cm3 <= 0.0 or self.material_shore_a != 83.0:
            raise ValueError("This V3.1 configuration is qualified only for the named TPE-83A.")


@dataclass(frozen=True)
class CouponConfig:
    """Nominal dimensions and mesh resolution for the printability box."""

    size_mm: float = 30.0
    preview_step_mm: float = 0.75
    production_step_mm: float = 0.50
    lateral_envelope_inset_mm: float = 0.0

    def validate(self) -> None:
        """Keep the coupon cubic and fine enough to resolve 1.5 mm walls."""
        if self.size_mm != 30.0:
            raise ValueError("The requested printability coupon is exactly 30 mm nominal.")
        if self.production_step_mm > 0.50:
            raise ValueError("The production coupon needs at least three wall samples.")
        if self.preview_step_mm < self.production_step_mm:
            raise ValueError("Preview resolution cannot exceed production resolution.")
