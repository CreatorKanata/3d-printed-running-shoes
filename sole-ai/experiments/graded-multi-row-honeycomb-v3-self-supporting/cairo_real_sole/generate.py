# cairo_real_sole.generate: build and verify the V3.1 FFF sole.
"""Emit a conformal honeycomb with no horizontal internal cell edges."""
...
from .plate_slot import build_plate_slot_field
...
def generate_real_sole(
    ...
    *,
    ...
    """Replace one real designer volume with a clipped multi-row honeycomb."""
    ...
    field_builder = build_plate_slot_field if config.plate_slot_enabled else build_field
    field = field_builder(axes, interval_columns, layout, config)
    ...
    output_stl.parent.mkdir(parents=True, exist_ok=True)
    write_binary_stl(output_stl, triangles, "Self-Supporting Honeycomb V3.1")
    ...
    report = {
        ...
        "architecture": "graded-multi-row-honeycomb-v3-self-supporting",
        "honeycomb_columns": layout.column_count,
        "honeycomb_rows": layout.row_count,
        "honeycomb_cells_covering_bounds": layout.cell_count,
        "minimum_nonvertical_wall_angle_deg": layout.minimum_rising_angle_deg,
        "horizontal_honeycomb_edge_count": layout.horizontal_edge_count,
        ...
        "design_note": (
            "Side openings are intentional. Point-top cells use vertical sides "
            "and diagonals rising more than 45 degrees; internal honeycomb "
            "segments parallel to the build plate are eliminated. Continuous "
            "1.5 mm upper/lower skins distribute landing load; two "
            "bottom-to-top vertical rails remove floating undersides and "
            "resist out-of-plane buckling. "
            "Heel and forefoot walls grade smoothly "
            "from 1.5 to 1.8 mm. "
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate the self-supporting V3.1 sole.")
