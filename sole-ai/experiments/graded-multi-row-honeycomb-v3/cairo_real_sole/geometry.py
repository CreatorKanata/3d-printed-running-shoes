# cairo_real_sole.geometry: conformal, graded, multi-row honeycomb field.
"""Fill the scanned sole with short, mutually braced hexagonal TPE walls."""
...
class HoneycombLayout:
    """Pre-binned flat-top hexagon edges covering the left sole side profile."""
...
def make_layout(
    ...
    """Create a four-row tiling; smoothly graded wall thickness changes density."""
    edge = config.honeycomb_edge_mm
    hex_height = math.sqrt(3.0) * edge
    y_step = 1.5 * edge
    y_origin = y_min + edge
    z_origin = z_min + hex_height / 2.0
    q_max = math.ceil((y_max - y_origin) / y_step) + 2
    r_max = math.ceil((z_max - z_origin) / hex_height) + 2
    unique_segments: dict[tuple[Point2, Point2], Segment2] = {}
    cells = 0
    columns = set()
    rows = set()
    for q_index in range(-2, q_max + 1):
        centre_y = y_origin + q_index * y_step
        stagger = (q_index & 1) * hex_height / 2.0
        for r_index in range(-2, r_max + 1):
            centre_z = z_origin + stagger + r_index * hex_height
            vertices = _hex_vertices(centre_y, centre_z, edge)
    ...
    return HoneycombLayout(
        ...
        row_count=max(1, round(len(rows) / 2.0)),
        cell_count=cells,
    )
...
def _hex_vertices(centre_y: float, centre_z: float, edge: float) -> list[Point2]:
    """Return a flat-top hexagon with steep printable diagonals."""
    half_height = math.sqrt(3.0) * edge / 2.0
    return [
        (centre_y + edge, centre_z),
        (centre_y + edge / 2.0, centre_z + half_height),
        (centre_y - edge / 2.0, centre_z + half_height),
        (centre_y - edge, centre_z),
        (centre_y - edge / 2.0, centre_z - half_height),
        (centre_y + edge / 2.0, centre_z - half_height),
    ...
    for index in range(1, config.longitudinal_rail_count + 1):
        ...
        rail = min(
            ...
            max(
                ...
                layout.y_min + config.honeycomb_edge_mm - y_mm,
                y_mm - (layout.y_max - config.honeycomb_edge_mm),
